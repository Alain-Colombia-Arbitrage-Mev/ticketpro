  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AdminEventRow[]> => {
      let q = supabase
        .from("events")
        .select(
          "id, title, date, time, location, category, description, image_url, base_price, currency, is_active, featured, trending, sold_out, last_tickets, metadata, created_at, updated_at",
        )
